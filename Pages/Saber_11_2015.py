"""
Dashboard ICFES Saber 11 – 2015 con caché en disco (página dentro de multi-page app)
===================================================================
Registra la página en /2015 y expone `layout` para que Dash la detecte.
...
Para forzar reprocesamiento manual:
    python pages/Saber_11_2015.py --rebuild
"""
...
# REGISTRO DE PÁGINA
# ─────────────────────────────────────────────────────────────
dash.register_page(__name__, path="/2015", name="Saber 11 · 2015")

# ─────────────────────────────────────────────────────────────
# CONFIGURACIÓN
# ─────────────────────────────────────────────────────────────
BASE_PROYECT_DIR = Path(__file__).resolve().parents[2]
CSV_PATH   = BASE_PROYECT_DIR / "Datasets" / "Data_ICFES" / "Saber_11" / "Unificados" / "Limpios" / "Saber_11_2015_filtrado.csv"
CACHE_DIR  = Path("Cache")
CACHE_FILE = CACHE_DIR / "Saber11_2015_cache.pkl"
...
def build_cache(csv_path: Path) -> dict:
    print("=" * 55)
    print("  Primera ejecución: procesando CSV 2015…")
    print("=" * 55)
    t0 = time.time()
    ...
    # Pilo paga
    pilopaga_vc = df["estu_pilopaga"].value_counts()
    ...
    figs["pilopaga"]     = pie_fig(
        list(pilopaga_vc.values),
        lmap(pilopaga_vc, {"S": "Con beca Pilo Paga", "N": "Sin beca Pilo Paga"})
    )
...
def load_or_build(force=False) -> dict:
    if not force and CACHE_FILE.exists():
        print(f"  Cargando caché 2015 desde {CACHE_FILE}…", end=" ", flush=True)
        t0 = time.time()
...
def build_layout(data: dict):
    ...
    return html.Div(style={
        ...
        html.Div([
            html.Div([
                html.Div("ICFES · 2015", style={"color": ACCENT1, "fontSize": "11px", "letterSpacing": "4px"}),
        ...
        # ── 9. Pilo Paga ──
        card([
            section_title("Beca Pilo Paga"),
            graph(figs["pilopaga"], "300px"),
        ]),
        ...
        # ── Footer ──
        html.Div("ICFES Saber 11 · 2015 · Análisis de datos educativos",
